...
from snp_haplotyper.snp_haplotype import main
import pytest
import json
import re
...
@pytest.mark.parametrize("name", setup_test_data())
def test_informative_snps(capsys, name):
    args = setup_test_data()
    main(args[name])
    output = capsys.readouterr().out
    results = json.loads(output)
    json_file_path = "test_data/autosomal_dominant/informative_snp_validation.json"
